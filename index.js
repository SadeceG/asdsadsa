...
.setThumbnail()
.addField(`🤖 BT-A | Teşekkürler`, `Merhaba, Ben BT-A (BT-A Geliştiricisi) 🤖\nÖncelikle Beni Eklediğiniz ve Bize Destek Verdiğiniz İçin Teşekkür Ederim!`)
.addField(`🤖 BT-A | Prefix`, `BT-A Bot'un Prefix'i (ön eki) = \`${fynx.prefix}\`\nDeğiştirmek için \`${fynx.prefix}prefix\` komutunu kullanabilirsiniz.`)
.addField(`🤖 BT-A | Nasıl Kullanılır?`, `BT-A bot'un tüm özelliklerinden yararlanmak için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.`)
.addField(`🤖 BT-A | Linkler`, `Sohbet Kanalına ".davet" yazarak davet linkine ulaşabilirsiniz.`)
.setFooter(`🤖 BT-A | Gelişmiş Türkçe Bot | 2023`)
.setTimestamp();
...
var oyun = [
`✨ Yardım almak için | bta!yardım`,
`🚀 Yeni Özellikler İçin | bta!yardım`,
`🔔 Created by efesbeys'`,
`🔔 Created by BTA0607'`,
`⚡️ Botu eklemek için | bta!davet`,
`🌟 Prefix ayarlamak için | bta!prefix`
]
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!847797279507218443>`) return msg.channel.send(`> **BT-A | Prefix**\n\n> <a:cubugk:1170424561116905552> **Sanırım beni etiketlediniz.**\n >  <a:cubugk:1170424561116905552 Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(`<:diamond63:1170467558319734795> <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("BT-ABot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffd100")
    ...
    .setDescription(`<:diamond63:1170467558319734795> **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
        .setThumbnail(message.user.avatarURL)
  .setFooter("BT-A Bot | Log Sistemi")
  modlogkanal.send(embed);
  })
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setDescription(`<:diamond63:1170467558319734795> ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`)
                .setFooter(`BT-A Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`<:diamond63:1170467558319734795> ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`)
                .setFooter(`BT-A Bot | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.MessageEmbed()
                    .setColor('#ffd100')
                .setDescription(`<:diamond63:1170467558319734795> ${channel.name} **Adlın Metin Kanalı  Silindi**`)
                .setFooter(`BT-A Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`<:diamond63:1170467558319734795> ${channel.name} **Adlı Ses Kanalı Silindi**`)
            .setFooter(`BT-A Bot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
    ...
    if (küfür.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<:diamond63:1170467558319734795> **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<:diamond63:1170467558319734795> **Hey ${newMessage.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<:diamond63:1170467558319734795> **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<:diamond63:1170467558319734795> **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:welcome:1170514383189835977>`"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }

  if (member.guild.memberCount == sayaç) {
    return client.channels.get(kanal).send(`<a:hypesquad1:763723597863714857> **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`)
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<:ok1:1170436486097817680> `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:welcome:1170514383189835977> **Selam!** `" + member.user.username + "`**bta!kayıtol yazarak kayıt olabilirsin!**");
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:welcome:1170514383189835977> `" + member.user.username + "`** Hoş Geldin! Otomatik Rolün Verildi Seninle Beraber** `" + member.guild.memberCount + "` **Kişiyiz!**");
